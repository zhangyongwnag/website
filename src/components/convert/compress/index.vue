<template>
  <el-scrollbar class="scroll_wrap" :native="false" wrapStyle="overflow-x: hidden;" wrapClass="" viewClass="" viewStyle="" :noresize="false" tag="section">
    <div class="convert_wrap">
      <div class="_common_block _flex_column_center" v-for="(item,index) in compress_list" @click="handle_click(item,index)">
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      // 过滤压缩列表
      compress_list() {
        return this.convert_list.filter(item => item.type == 'compress')
      },
    },
    methods: {
      /**
